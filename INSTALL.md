...
dsh web **设置 → AomeRAG 知识库**:9 项参数表单(切片/检索参数与 Ollama 地址、模型热更新;「启动时自动同步」开关下次启动生效)、库状态快照(文档数/chunk 数/最后同步/同步报告/库体积,**知识目录与库路径只读展示**)与四个操作按钮(立即同步/重建索引/清空库/打开知识目录)。知识目录与库路径是部署字段:修改走上方 `cordis.patch.yml` 并重启。状态为快照式,同步事件自动刷新;对话通道(kb_status/kb_ingest)保持可用。
