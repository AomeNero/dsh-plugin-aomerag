...
describe('KbStore: upsertDoc 与 chunkMeta', () => {
  ...
  it('跳过不存在的 rowid(并发同步删除场景的宽容行为)', () => {
    store.upsertDoc('a.md', [chunk('x')], [vec(1)])
    const id = store.knn(vec(1), 1)[0]!.rowid
    const metas = store.chunkMeta([id, id + 999])
    expect(metas).toHaveLength(1)
    expect(metas[0]!.content).toBe('x')
  })

  it('chunks 与 vectors 数量不符时抛错', () => {
    expect(() => store.upsertDoc('a.md', [chunk('x')], [])).toThrow()
  })

  it('空块列表的 upsertDoc 等于清空该文档', () => {
    store.upsertDoc('a.md', [chunk('x'), chunk('y')], [vec(1), vec(0, 1)])
    store.upsertDoc('a.md', [], [])
    expect(store.docCount()).toEqual({ docs: 0, chunks: 0 })
  })
})
...
describe('KbStore: FTS 中文', () => {
  ...
  it('rank 升序且受 k 截断', () => {
    store.upsertDoc('m.md', [chunk('电源 电源 电源 电源'), chunk('电源')], [vec(1), vec(1)])
...
describe('KbStore: 删除与幂等', () => {
  it('deleteDoc 清空向量/元数据/FTS 三表', () => {
    store.upsertDoc('a.md', [chunk('电源内容'), chunk('别的')], [vec(1), vec(0, 1)])
    store.deleteDoc('a.md')
    expect(store.docCount()).toEqual({ docs: 0, chunks: 0 })
    expect(store.knn(vec(1), 5)).toEqual([])
    expect(store.fts(tokenize('电源'), 5)).toEqual([])
  })

  it('同一文档重复 upsert 先删后插,无残留', () => {
    store.upsertDoc(
      'a.md',
      [chunk('旧一'), chunk('旧二'), chunk('旧三')],
      [vec(1), vec(0, 1), vec(0, 0, 1)],
    )
    store.upsertDoc('a.md', [chunk('新一')], [vec(1)])
    expect(store.docCount()).toEqual({ docs: 1, chunks: 1 })
    expect(store.chunkMeta(store.knn(vec(1), 10).map((h) => h.rowid))[0]!.content).toBe('新一')
    expect(store.fts(tokenize('旧一'), 10)).toEqual([]) // 旧 FTS 已随先删清理
  })
})

describe('KbStore: fileRegistry', () => {
  it('未登记的 docId 返回 undefined', () => {
    expect(store.fileRegistry.get('nope.md')).toBeUndefined()
  })

  it('set 后可 get,重复 set 覆盖', () => {
...
describe('KbStore: 持久化', () => {
  it('文件库重开:数据仍在,vec0 扩展重载,knn/fts 可用', () => {
